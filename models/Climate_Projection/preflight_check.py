#!/usr/bin/env python3
"""
Preflight check for Climate Projection (CMIP6) — verifies environment before simulation.
...
def main():
    global PASS, FAIL
    print(f"=" * 60)
    print(f"  PREFLIGHT CHECK: Climate Projection (CMIP6)")
    print(f"=" * 60)
    print()

    # Model-specific checks
    # Python package: xarray (NetCDF handling)
    check_import("xarray", "xarray (NetCDF handling)")
    # Python package: scipy (interpolation)
    check_import("scipy", "scipy (interpolation)")
    # Directory: Baseline forcing data
    check_dir("KISSPATH_FORCING", "Baseline forcing data")
