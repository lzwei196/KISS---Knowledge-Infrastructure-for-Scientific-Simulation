#!/usr/bin/env python3
"""
SWAT+ VERIFIER — Bengbu (蚌埠), Huai River, gauge 51080.

Second location for the SWAT+ consistency check. The Real-case stage ran the
same KI chain at Zijingguan (Juma/Haihe, 1767 km2, semi-arid Taihang front).
This script runs the IDENTICAL chain, from scratch, at Bengbu: the Huai main
stem at the Bengbu sluice (32.9504N, 117.3325E), published drainage area
121,330 km2 — a humid-subtropical basin ~69x larger.
...
DEM = WORK / "elv_clip.tif"        # MERIT Hydro elevation, clipped to the Huai
DIR_TIF = WORK / "dir_clip.tif"    # MERIT Hydro D8 flow direction (conditioned)

OUTDIR = ROOT / "models/SWAT+/detached/verify_1"
RESULT = OUTDIR / "result.json"
TRIALS = OUTDIR / "trials.json"

# Bengbu sluice (蚌埠闸). Located as the MERIT `upa` main-stem cell whose upstream
# area matches the gauge's published 121,330 km2 (MERIT reads 123,029 there, +1.4%).
OUTLET_LAT, OUTLET_LON = 32.9504, 117.3325
PUBLISHED_AREA_KM2 = 121330.0
...
N_SUBBASINS = 8
STREAM_THRESHOLD_KM2 = 500.0           # scaled to a 121,330 km2 basin
SNAP_DIST_DEG = 0.01
...
# must be regenerated. detached/verify_1/ already held a completed result.json from
# 2026-06-28 (NSE 0.7518) produced by run_and_score_v1b.py on a PRE-CALIBRATED Bengbu
# deck over 1981-1990 — resuming onto it would have reported a tuned deck's skill as
# this from-scratch verifier's, and its period string even shares the 1981 prefix.
RUN_SIGNATURE = f"bengbu_v1|from_scratch|{SIM_START}..{SIM_END}|cal={CAL[0]}..{CAL[1]}"
...
def build_basin():
    ...
    TOOLS_FAILED.append(
        ...
        f"across the Huaibei plain / Grand Canal, so the pour point captures (or loses) whole "
        f"sub-basins — dt_008 with no fatal error. WORKAROUND used here: trace the basin through "
        f"MERIT Hydro's hydrologically-CONDITIONED D8 `dir` grid, which reproduces the published "
        f"121,330 km2 to +0.3%. The KI has no tool for this; s1 should accept a conditioned "
        f"flow-direction raster instead of re-deriving one from raw elevation on low-relief basins."
    )
    if shp.exists():
        shp.unlink()
...
def build_txtinout(basin_shp):
    ...
    tool("s2/generate_hru_from_global.py",
         ...
         "--output_dir", TXTINOUT, "--basin_name", "bengbu",
...
def write_result(**kw):
    ...
    base = {"model_id": "SWAT+", "this_location": "Bengbu", "obs_source": "ObservedQ",
...
def main():
    ...
    f = m["full"]
    notes = (
        f"SWAT+ rev59 built FROM SCRATCH at Bengbu (Huai 51080, outlet {OUTLET_LAT}N "
        f"{OUTLET_LON}E, delineated {area:.0f} km2 vs published {PUBLISHED_AREA_KM2:.0f}), "
        f"same KI chain as the Zijingguan real case. CMFD 3-hourly forcing, {len(names)} "
...
if __name__ == "__main__":
    try:
        ...
        write_result(
            ...
            notes=f"SWAT+ Bengbu verifier failed: {e}",
            log_tail=LOG[-60:])
        raise
