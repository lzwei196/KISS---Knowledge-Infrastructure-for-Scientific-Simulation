#!/usr/bin/env python3
"""Build pinned native FATES hosted by CTSM only; never submit/run a case or acquire inputdata."""
import argparse
import json
import os
...
def main():
    ...
    parser.add_argument('--runtime', default=os.environ.get('FATES_NATIVE_RUNTIME'))
    parser.add_argument('--python', default=os.environ.get('FATES_BUILD_PYTHON'))
    ...
    config = {}
    for config_path in (source.parent.parent / 'fates-build-config.json', source / 'fates-build-config.json'):
        if config_path.is_file():
            config.update(json.loads(config_path.read_text()))
    native_path = args.runtime or config.get('native_runtime')
    if not native_path:
        raise SystemExit('Set --runtime or fates-build-config.json native_runtime to the genuine ESMF/PIO/MPICH/netCDF prefix')
    ...
    (source / 'fates-build-config.json').write_text(json.dumps({'native_runtime': str(runtime), 'build_python': str(python)}, indent=2) + '\n')
    env = os.environ.copy()
    env.update(PATH=str(python.parent) + ':' + str(runtime / 'bin') + ':/opt/homebrew/bin:' + env.get('PATH', ''),
               FATES_NATIVE_RUNTIME=str(runtime), MPICH_FC='/opt/homebrew/bin/gfortran',
    ...
    if args.build_stage:
        ...
        for sub, pin in SUBPINS.items():
            if subprocess.check_output(['git', '-C', str(source / sub), 'rev-parse', 'HEAD'], text=True).strip() != pin:
        ...
        casepath = source / 'ki-fates-case'
        os.chdir(casepath)
        with Case(str(casepath), read_only=False, record=True) as case:
            if 'CLM50%FATES' not in case.get_value('COMPSET'):
                raise SystemExit('Case is not the declared native FATES configuration')
    ...
    def run(argv, cwd=source):
        ...
        logfile = source / ('fates-build-step-' + str(len(commands)) + '.log')
    ...
    paths = {'CIME_OUTPUT_ROOT': source / 'ki-fates-output', 'DIN_LOC_ROOT': source / 'ki-fates-inputdata',
             'DIN_LOC_ROOT_CLMFORC': source / 'ki-fates-inputdata', 'DOUT_S_ROOT': source / 'ki-fates-archive',
             'BASELINE_ROOT': source / 'ki-fates-baselines', 'CCSM_CPRNC': source / 'ki-fates-tools/cprnc'}
    ...
    case = source / 'ki-fates-case'
    if not case.exists():
        run([python, source / 'cime/scripts/create_newcase', '--case', case,
             '--compset', 'I2000Clm50FatesRsGs', '--res', 'f19_g17', '--machine', 'homebrew',
             '--compiler', 'gnu', '--mpilib', 'mpich', '--pecount', '1', '--run-unsupported'])
    run([python, case / 'xmlchange', ','.join(['EXEROOT=' + str(source / 'ki-fates-build'),
         'RUNDIR=' + str(source / 'ki-fates-run'), 'DIN_LOC_ROOT=' + str(source / 'ki-fates-inputdata'),
         'DIN_LOC_ROOT_CLMFORC=' + str(source / 'ki-fates-inputdata'), 'GET_REFCASE=FALSE', 'GMAKE_J=2'])], case)
    run([python, case / 'case.setup'], case)
    if not args.configure_only:
        run([python, Path(__file__).resolve(), source, '--runtime', runtime, '--python', python, '--build-stage'])
    receipt = {'source_commit': PIN, 'compset': 'I2000Clm50FatesRsGs', 'fates_commit': SUBPINS['src/fates'], 'submodules': SUBPINS, 'runtime': str(runtime),
               'python': str(python), 'commands': commands, 'configure_only': args.configure_only,
               'native_product': str(source / 'ki-fates-build/cesm.exe'),
               'scientific_execution': False, 'inputdata_download': False}
    (source / 'fates-build-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
    if not args.configure_only and not (source / 'ki-fates-build/cesm.exe').is_file():
