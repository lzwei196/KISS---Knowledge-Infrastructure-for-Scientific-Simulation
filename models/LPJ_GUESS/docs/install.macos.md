# LPJ_GUESS: declared Python variant on macOS

This recipe installs **lpj-guess-analytic**, the implementation already declared in both the Mac and Windows KI metadata. Its version description is `HydroCraft analytic reimplementation`. It does not install the official upstream LPJ_GUESS application or establish scientific equivalence with it.

The real engine is the existing bundled `tools/run_lpjguess.py`. Its SHA256 is `44b7677f4294f26257f58f5d690d6b6751aa7eb32d79ed5324d9863e42a16c04`. No runner code or equations were changed for this installation contract. The script contains no path-substitution tokens; the materialized workspace copy must retain these exact bytes.
