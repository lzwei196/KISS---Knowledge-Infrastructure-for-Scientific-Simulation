#!/usr/bin/env python3
"""APEX VERIFIER (verify_1): continuous maize at a North-China-Plain maize-belt
cell (Henan, 35.0N 114.5E) -- a genuinely DIFFERENT location than the real-case
Bengbu Huai cell (32.75N 117.25E) -- scored against the FAOSTAT national
"China, mainland" Maize (corn) yield series.
...
Faithful twin of the real-case runner (run_and_score.py):
  ...
  * Same Y10RF management (validated CORN BENGBU schedule with the full-auto
    ...
    wired to every subarea -> continuous corn.
  * ONLY the location changes: CMFD forcing rebuilt at 35.0N/114.5E and the SIT
    lat/lon/elev updated so daylength/PHU match the new cell.  CMFD 1961-2016
    (20-yr spin-up 1961-1980), scoring window 1981-2016, ngn=0.

Obs = FAOSTAT China maize yield (kg/ha -> t/ha).  This is a NATIONAL aggregate
(regional_aggregate_time_series) carrying a strong technology trend the fixed-
management APEX run cannot track -> determining metric is pbias (magnitude);
detrended r reported for pattern only.
...
def forcing_ready():
    ...
    if not dly.is_file():
        # fall back: any *.dly with our year span
...
def load_faostat():
    """FAOSTAT China mainland Maize (corn) yield, kg/ha -> t/ha, keyed by year."""
    series = {}
    with open(FAOSTAT, newline="") as f:
        for row in csv.DictReader(f):
            if row.get("Item", "").strip().lower().startswith("maize") and \
               row.get("Element", "").strip() == "Yield":
                try:
                    yr = int(row["Year"])
                    val = float(row["Value"])
                except (ValueError, KeyError):
                    continue
                series[yr] = val / 1000.0   # kg/ha -> t/ha
    return series
...
def main():
    ...
    # relocate site (daylength/PHU) to the NCP cell
    try:
    ...
    result = {
        ...
        "obs_source": "FAOSTAT China Crop Yields",
        "status": "completed",
        ...
        "notes": ("Verifier twin of the Bengbu real-case: identical Y10RF rainfed CORN "
                  ...
                  "DIFFERENT cell (NCP Henan 35.0N,114.5E) and scored vs the FAOSTAT national "
                  "China maize yield (kg/ha->t/ha). National FAOSTAT is a strongly-trended "
                  "aggregate the fixed-management run cannot track -> pbias is the determining "
                  "metric (magnitude); detrended r reported for pattern only."),
    }
