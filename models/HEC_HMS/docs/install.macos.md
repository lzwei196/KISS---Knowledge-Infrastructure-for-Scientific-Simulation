# HEC_HMS: declared Python variant on macOS

This recipe installs **hec-hms-python-equiv**, the implementation already declared in both the Mac and Windows KI metadata. Its version description is `HydroCraft Python equivalent of HEC-HMS 4.x lumped methods`. It does not install the official upstream HEC_HMS application or establish scientific equivalence with it.

The real engine is the existing bundled `tools/run_hec_hms.py`. Its SHA256 is `172f5301a25757b39597ed8c0d2350669e4a960e82cb1fa2d247440dd32208b5`. No runner code or equations were changed for this installation contract. The script contains no path-substitution tokens; the materialized workspace copy must retain these exact bytes.
