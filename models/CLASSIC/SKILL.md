...
|---|---|
| `tools/convert_forcing_to_classic.py` | `KISSPATH_PYTHON_ENV/bin/python {KI}/tools/convert_forcing_to_classic.py --help` |
| `tools/convert_soil_to_classic.py` | `KISSPATH_PYTHON_ENV/bin/python {KI}/tools/convert_soil_to_classic.py --help` |
| `tools/parse_classic_output.py` | `KISSPATH_PYTHON_ENV/bin/python {KI}/tools/parse_classic_output.py --help` |
| `tools/run_classic.py` | `KISSPATH_PYTHON_ENV/bin/python {KI}/tools/run_classic.py --help` |
...
**Headline output**: `gpp` — Terrestrial ecosystem gross primary productivity (`kgC m-2 s-1 (instantaneous) or kgC m-2 month-1 (accumulated)`)

| Output variable (dag `var`) | Validation rank | Unit | Description |
|-----------------------------|-----------------|------|-------------|
| gpp | 1 | kgC m-2 s-1 (instantaneous) or kgC m-2 month-1 (accumulated) | Terrestrial ecosystem gross primary productivity |

Other dag outputs: `rss`, `hfss`, `hfls`, `nep`, `nbp`, `rh`, `mrro`, `snw`, `lai`, `cSoil`.

---

## Unit Table

This dag-sourced output unit table is the post-processing contract for the headline output. Use `dag.yaml` for the complete machine-readable output inventory.

| Output variable | Unit | Notes |
|-----------------|------|-------|
| gpp | kgC m-2 s-1 (instantaneous) or kgC m-2 month-1 (accumulated) | Rank-1 output; Terrestrial ecosystem gross primary productivity |

The older output table below keeps common CLASSIC output names for operator convenience, but the dag-sourced entry above is authoritative for the rank-1 output.

---

## Validated Results

Performance verdicts are judged against `docs/validation_convention.yaml`, not intuition. The convention bars currently sourced for this KI are:

| Output variable | Metric | Direction | Satisfactory | Good | Very good | Citation key |
|-----------------|--------|-----------|--------------|------|-----------|--------------|
| rss | nse | maximize | 0.5 (moriasi2015) | 0.7 (moriasi2015) | 0.8 (moriasi2015) | moriasi2015 |
| rss | csi | maximize | no cited threshold | no cited threshold | no cited threshold | none |
| hfss | nse | maximize | 0.5 (moriasi2015) | 0.7 (moriasi2015) | 0.8 (moriasi2015) | moriasi2015 |

No achieved calibration, validation, or full-period metric values are stated in this SKILL body. Record run-specific values only after executing the actual CLASSIC pipeline and scoring them against the cited bars above.
