#!/usr/bin/env python3
"""Preflight check for WSIMOD — auto-generated by KDT v5.0"""
...
def main():
    ...
    print()
    # WSIMOD in dissection venv
    import sys
    sys.path.insert(0, "/home/server/knowledge-dissection-toolkit/auto_dissect/_work/WSIMOD/venv/lib/python3.12/site-packages")
    # WSIMOD: use venv python (has deps system python lacks)
    import subprocess
    try:
        proc = subprocess.run(["/home/server/knowledge-dissection-toolkit/auto_dissect/_work/WSIMOD/venv/bin/python3", "-c", "import wsimod"], 
            capture_output=True, timeout=10)
        if proc.returncode == 0:
            print(f"  OK    WSIMOD (wsimod): verified via venv python")
            PASS += 1
        else:
            print(f"  FAIL  WSIMOD (wsimod): {proc.stderr.decode().strip().split(chr(10))[-1]}")
            FAIL += 1
    except Exception as e:
        print(f"  FAIL  WSIMOD (wsimod): {e}")
        FAIL += 1
    # Check diagnostics
