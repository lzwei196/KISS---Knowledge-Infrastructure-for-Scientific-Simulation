#!/usr/bin/env python3
"""Preflight check for SimPEG — auto-generated by KDT v5.0"""
...
def main():
    ...
    import subprocess
    try:
        proc = subprocess.run(["/home/server/knowledge-dissection-toolkit/auto_dissect/_work/SimPEG/venv/bin/python3", "-c", "import SimPEG"],
            capture_output=True, timeout=10)
        if proc.returncode == 0:
            print(f"  OK    SimPEG: verified via venv python")
            PASS += 1
        else:
            print(f"  FAIL  SimPEG: {proc.stderr.decode().strip().split(chr(10))[-1]}")
            FAIL += 1
    except Exception as e:
        print(f"  FAIL  SimPEG: {e}")
        FAIL += 1
    # Check diagnostics
