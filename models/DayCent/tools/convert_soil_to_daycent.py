...
canonical layout used by rev 491 (cross-checked against the WoosterExampleLinux
file shipped with the binary distribution):
    ...
    8: sand fraction     (0..1)
    9: clay fraction     (0..1)
   10: organic matter    (fraction of mass, 0..1)
   11: saturated K       (cm/sec)
   12: pH                (unitless)
    ...
    with open(out_path, "w") as f:
        for r in rows:
            f.write(
                ...
                f"{r[7]:.2f}\t{r[8]:.2f}\t{r[9]:.2f}\t{r[10]:.5f}\t{r[11]:.2f}\n"
            )
...
def main(argv=None):
    ...
    soil_top = lookup_hwsd(args.lat, args.lon, depth="topsoil")
    soil_sub = lookup_hwsd(args.lat, args.lon, depth="subsoil")
