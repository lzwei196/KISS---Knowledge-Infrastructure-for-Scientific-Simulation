...
Virginia Falls is:
  * a different continent, forcing dataset (NASA POWER, not CMFD) and DEM
    (Copernicus GLO-30, not china_dem_90m);
  * 15,379 km2 -- above forcing_nasa_power.py's 10,000 km2 recommendation,
    matching the John Day (verify_2) scale;
  * genuinely UNREGULATED -- it drains Nahanni National Park Reserve (Mackenzie
    ...
    hydrograph is a fair test (the same rationale that picked John Day);
  * a COLD snowmelt regime (frac_snow 0.44, subarctic 61.6N) -- maximally
    distinct from the tropical monsoon Jinghong real-case, so the snow/soil
    physics is being probed in a regime the real case never saw;
  * daily obs are 100% complete 2003-2015 (no gaps, cf. dt_vic_021).
...
--------------------------------------------------------------------------
  ...
  s1b build elevation + annual-precip NetCDFs on the VIC grid (the CMFD ones
      are China-only; fill_parameters1.py takes VIC_ELEV_NC / VIC_PREC_ANNUAL_NC
  ...
  s5  s2_forcing/forcing_nasa_power.py                      -> per-cell VIC ASCII forcing
      (3-hourly, 8 steps/day, 7 cols in FORCE_TYPE order; CMFD does not reach Canada)
...
Periods (obs are complete; NASA POWER starts 2001; a 3-year spin-up is used
because a subarctic snow/soil column equilibrates slowly, cf. dt_vic_030):
...
and the SAME standard (non-frozen) global-param template the Jinghong real case
and the John Day verifier used, so the metrics are directly comparable. This
basin is cold enough that FROZEN_SOIL would matter (cf. Harbin / dt_vic_031); the
uncalibrated non-frozen protocol is kept deliberately for comparability and any
residual is reported, not tuned away.
...
native units and the metrics are directly comparable to Jinghong's.
...
if not (os.path.exists(ELEV_NC) and os.path.exists(PREC_NC)):
    ...
    # annual_prec (soil col 49) is read by the image driver only and never used by
    # vic_run in the classic driver, but a physical value beats -9999.
...
# fill_parameters1 writes zeros for Ksat/expt/bulk density; fill_parameters2 fills
# them from the global 0.5deg file. If that interpolation missed, VIC would run
# with Ksat = 0 and produce a silently wrong hydrograph.
...
# POWER (0.5deg, hourly, 2001-NRT) is the sanctioned global fallback for a
# >10,000 km2 basin outside the CMFD/MSWX domains.
...
if have < NCELL:
    ...
    run(f"{KI}/s2_forcing/forcing_nasa_power.py", f"{KI}/s2_forcing",
        "s2_forcing/forcing_nasa_power.py", timeout=43200,
...
save(
    ...
    f"irrelevant) -> s4_veg (AVHRR global) -> s2_forcing/forcing_nasa_power.py (3-hourly, 8 "
    f"steps/day; CMFD does not reach Canada) -> vic_classic.exe -> build_routing_param.py -> "
    f"route_1.0/rout. paired={len(paired)} d ({EVAL_START}..{EVAL_END}); "
    ...
    f"module. CAVEAT: this subarctic basin is cold enough that FROZEN_SOIL would matter "
    f"(cf. Harbin/dt_vic_031) and 3-yr spin-up may still under-equilibrate the deep store "
    f"(dt_vic_030); the standard non-frozen uncalibrated protocol is kept deliberately for "
    f"comparability with the real case, and any residual snow-timing/volume bias is reported, "
    f"not tuned away."
)
