...
# ---------------------------------------------------------------------------
# =============================================================================
# calib_run_wangjiaba.py — SYSTEM 1 PHASE-1 PROBE VARIANT of tools/calib_run.py.
# IDENTICAL machinery (inject/run/route/score/run-health, all fail-closed); ONLY the
# case-constants block below differs: case=SITE:wangjiaba, gauge CNGF 50101100 王家坝,
# 35-cell domain, per-EVENT splits, snow trio removed. Select the event with
# KDT_CALIB_SPLIT=<year> (1982|1984|1991|1996|1998). The tangnaihai calib_run.py is
# untouched. Regenerate by re-splicing this block if calib_run.py's machinery changes.
# =============================================================================
BASE = "KISSPATH_ROOT"
BASIN, STA = "wangjiaba", "WJB"
# The VIC-Wangjiaba domain is NOT under outputs/<basin>/ like tangnaihai; it is the
# validated real_case build (35 cells; soil/veg/global/forcing) with its own dir name.
BDIR = f"{BASE}/outputs/wangjiaba_vic_realcase_2026_05_29"

SOIL_SRC = f"{BDIR}/vic_temp/soil/SOIL_PARAM_COMPLETE.txt"        # 35 rows x 53 cols
VEG_SRC = f"{BDIR}/vic_temp/veg/vic_veg_param_final.txt"
VEGLIB_SRC = f"{BASE}/data/vic_param/veglib.LDAS"                 # shared library (as tangnaihai)
GP_SRC = f"{BDIR}/vic_temp/global_param_wangjiaba_vic_realcase_2026_05_29.txt"
FORCING_SRC = f"{BASE}/outputs/wangjiaba_probe/vic_temp/forcing/forcing_final"  # EXTENDED 1980-1998 (P2)
FORCING_PREFIX = "wangjiaba_0.25deg_"                            # from the built global_param FORCING1
# Lohmann routing to the WJB pour point is a SEPARATE prepared dir (WJB_*.txt + UH.all +
# rout_global.txt), NOT BDIR/routing_param.
ROUT_SRC = f"{BASE}/outputs/vic_wangjiaba_routed_1981_1985"

# Caches are BASIN-SPECIFIC. uh_cache_path is keyed on (velocity,diffusivity) TEXT ONLY,
# so sharing the cache root with tangnaihai (same default v=1.5/D=800) would hand this
# basin tangnaihai's unit-hydrograph grid. Keep both caches under a per-basin root.
UH_CACHE_DIR = os.environ.get("KDT_VIC_UH_CACHE", f"/dev/shm/kdt_vic_calib/uh_s_{BASIN}")

VIC_EXE = f"{BASE}/model/VIC-5.1.0/vic/drivers/classic/vic_classic.exe"
ROUT_EXE = f"{BASE}/model/route_1.0/src/rout"

# ---------------------------------------------------------------------------
# TARGET CASE — pinned, not configurable (System 1 Phase-1 probe: 王家坝 Wangjiaba).
# load_obs() re-derives the station identity FROM THE FILE and refuses to score any
# other gauge, exactly as the tangnaihai driver does.
# ---------------------------------------------------------------------------
OBS_FILE = "KISSPATH_DATA/china_water_level/淮河txt/王家坝.txt"

TARGET_CASE = {
    "case_id": "SITE:wangjiaba",
    "dataset": "CNGF (china_gaugeflux) 淮河txt",
    "obs_file": OBS_FILE,
    "station_code": 50101100,
    "station_name": "王家坝",
    "variable": "Q",
    "units": "m3 s-1",
...
CASE_BY_STATION = {50101100: "SITE:wangjiaba"}

# Per-EVENT obs envelope (the probe scores ONE flood year per split). Generated by
# MIRRORING load_obs() slicing (q>-90 then loc[s0:s1]) on the CNGF 王家坝 series, at full
# float precision so the rel_tol 1e-9 envelope asserts pass — Wangjiaba Q is non-integer
# (unlike tangnaihai's integer Q). Provenance: models/vic/calibration.yaml obs_contract.
OBS_ENVELOPE = {
    "1982": {"n_days": 365, "q_min": 8.85, "q_max": 2550.0, "q_sum": 116324.09999999999, "q_mean": 318.6961643835616},
    "1984": {"n_days": 366, "q_min": 31.0, "q_max": 1810.0, "q_sum": 127464.3, "q_mean": 348.26311475409835},
    "1991": {"n_days": 365, "q_min": 32.2, "q_max": 2850.0, "q_sum": 125223.0, "q_mean": 343.07671232876714},
    "1996": {"n_days": 366, "q_min": 15.1, "q_max": 2730.0, "q_sum": 112167.7, "q_mean": 306.4691256830601},
    "1998": {"n_days": 365, "q_min": 41.6, "q_max": 2630.0, "q_sum": 140906.7, "q_mean": 386.04575342465756},
}

FORCING_CACHE = f"/dev/shm/kdt_vic_calib/forcing_{BASIN}_ext1998"   # bumped: extended forcing != old cache

# Per-EVENT sim/score windows. sim = 2 spin-up years + the scored flood year; score = the
# flood's full calendar year (scored via KDT_CALIB_SPLIT=<year>). Forcing covers 1980-1998
# (P2 extension done 2026-08-13, outputs/wangjiaba_probe) — all five events runnable.
SPLITS = {
    "1982": {"sim": (1980, 1982), "score": ("1982-01-01", "1982-12-31")},
    "1984": {"sim": (1982, 1984), "score": ("1984-01-01", "1984-12-31")},
    "1991": {"sim": (1989, 1991), "score": ("1991-01-01", "1991-12-31")},
    "1996": {"sim": (1994, 1996), "score": ("1996-01-01", "1996-12-31")},
    "1998": {"sim": (1996, 1998), "score": ("1998-01-01", "1998-12-31")},
}
...
# name -> default. MUST stay in sync with models/vic/calibration.yaml `parameters:`.
# The snow-albedo trio is DROPPED (inert on a warm monsoon basin); with SNOW_KEYS empty
# below, inject_constants writes an EMPTY constants file and VIC uses its built-in snow
# defaults. A handed key outside this set is fatal (unknown-parameter guard in main()).
DEFAULTS = {
...
def main() -> int:
    ...
    # Phase-2 state-vs-forcing probe: optional spin-up-start override (additive, env-guarded;
    # default behaviour unchanged). Lets us run the SAME scored year with a longer equilibration.
    # GUARDED (kimi review): the built forcing starts 1980 — an earlier year must fail fast
    # here, not as an opaque missing-forcing VIC error mid-run.
    _ss = os.environ.get("KDT_VIC_SPINUP_START")
    if _ss and int(_ss) < y0:
        if int(_ss) < 1980:
            raise Fail(f"KDT_VIC_SPINUP_START={_ss} precedes the built forcing (starts 1980)")
        y0 = int(_ss)
