...
def main():
    ...
    # WhiteboxTools resolves relative output paths against its OWN working
    # directory, not the caller's cwd. In pointer-mode d8_flow_accumulation
    # that silently writes fam.tif somewhere else, so the subsequent
    # rasterio.open(fam_out) fails with "No such file or directory" when
    # --out-dir is relative. Resolve to absolute up front so every derived
    # work/output path handed to WBT is absolute. (bug found 2026-06-22,
    # Missouri-basin real-case run.)
    out_dir = Path(args.out_dir).resolve()
    out_dir.mkdir(parents=True, exist_ok=True)
    args.dem = str(Path(args.dem).resolve())
