...
_PLAN_SCHEMA_NOTE = (
    ...
    "steps:[{id, ki, tool:'<absolute path under <KI>/tools/ or null>', kind, inputs:[inventory "
    ...
    "so only files that already exist can be 'ready'.\n"
)
...
def planning_block(kis: dict[str, Path], plan: dict, inventory: dict, project: Path,
    ...
    lines = [
        ...
        "pipeline tool. Non-mutating probes ARE allowed: --help, the KI's preflight_check.py, "
        "reading files. Approval and execution happen in a LATER, separate session — do not ask "
    ...
    lines.append(
        ...
        f"needs_user={s.get('asked_of_user')} coupling={[c.get('edge_id') for c in plan.get('coupling') or []]}\n"
