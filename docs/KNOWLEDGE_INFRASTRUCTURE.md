...
# Knowledge Infrastructure — the idea behind KISS
...
**Demonstration**: [https://Geoforgehhu.com](https://Geoforgehhu.com) · **Desktop app**: [README](../README.md)
...
> (121 in their `SKILL.md`, 54 as a real import in `tools/*.py`), and an install
> puts it into the model's environment. It also carries the **KI harness**, the
> one contract describing how an agent uses a KI; see
...
`/srv/models/...` for binaries, forcing archives, and DEMs, and
...
```bash
grep -rn "KISSPATH_" models/VIC/          # find what needs repointing    # find what needs repointing
